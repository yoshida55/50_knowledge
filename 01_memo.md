...
▢アニメーションについて、AOSなどを使うと上から下にドロップダウンできると思うのですが、
実際にはCSSでも同じようなことができると考えています。

ドロップダウンアニメーションなどを実装する際、以下のどちらの進め方が一般的でしょうか：

1. 自分たちが使いたい手法で自由に実装して良い
2. デザインカンプを渡された時点で、実装方法まで具体的に指示される
...
.menu_nav {
  display: flex;
  flex-wrap: wrap;
  align-content: space-between; /* nav行を上端・inquiry行を下端に */
}

/* 1行の中でのアイテム縦位置 */
.menu_nav {
  align-items: center; /* 各行の中でアイテムを縦中央に */
}
```
