...
・現在利用中のソフトバンク光の解約日が　5/20,5/31(解約工事日)
　6/9モバイルwifiのようなものを借りたい 
⇀　モバイルwifiのようなものを借りたい
　・料金
　2か月500円らしい
　・いつ
...
　・インターネットの接続までやってくれるサービスがあると認識したが
　して頂ける？工事だけ？？


⇀　ソフトバンクの違約金の支払い。

　・どのような手順で違約金などを支払ってもらえるのか？
...
ソフトバンク機器の返却先住所郵便番号:
 ...
 〒272-0001住所: 千葉県市川市二俣678-55 ESR市川ディストリビューションセンター3階 北棟N8宛名: ソフトバンク返品センター 宛
...
## 📌 カスタム投稿タイプのアーカイブ（一覧）をメニューに追加する正規の方法 →
CPT UIで「アーカイブあり」をON → 外観メニューの「表示オプション」でCPT名にチェック
→ 左パネルにアーカイブが出る → 選んで追加。カスタムリンクURL入力は非推奨 WordPress

【日付】2026-04-17 → 2026-05-04 更新

【結論】
カスタム投稿タイプの「アーカイブページ（一覧）」をメニューに追加するには2つの方法がある。
正規の方法はWordPressの管理画面から選んで追加する方法。

✅ **正規の方法（推奨）**
```
➀ CPT UI → 投稿タイプを編集 → 「アーカイブあり（has_archive）」をON → 保存
➁ 外観 → メニュー → 画面右上「表示オプション」をクリック
➂ カスタム投稿タイプ名（例：売上）にチェックを入れる
➃ 左パネルに投稿タイプ欄が出る → 展開するとアーカイブが選択肢に出てくる
➄ アーカイブにチェック → 「メニューに追加」→ メニューを保存
```
→ スラッグが変わっても自動追従。URLを覚える必要なし。
...
→ メニューに追加 → メニューを保存
```
→ スラッグが変わるとURLを手動で修正する必要がある。
→ has_archiveがOFFでも追加できてしまうが、ページが404になる可能性あり。

⚠ ローカル環境（Local）はカスタムリンクを使うとき `http://` を使う。
...
【functions.php との関係】
CPTの登録方法は2種類あるが、メニューへの追加方法（表示オプション）は同じ。

| 登録手段 | functions.php | 結果 |
|---------|--------------|------|
| CPT UI（プラグイン） | 書かなくてOK | has_archive をGUIで設定 |
| register_post_type() | 自分で書く | has_archive: true をコードで設定 |

→ どちらも「表示オプション → アーカイブ選択」で追加できる。登録手段が違うだけ。

⚠ CPT UI を使っている間は functions.php に register_post_type を書かなくてよい。
　実務では納品時に「CPT UIなしで動かす」ために functions.php へ移植することがある。
...
## width は固定の命令／max-width は上限のガード（スマホで違いが出る） HTML

【日付】2026-04-28

【結論】
`width: 600px` は「絶対この幅！」の命令 → スマホで画面より広いと **横スクロール発生**。
`max-width: 600px` は「ここまでなら広げてOK」の上限 → スマホでは画面に合わせて自動で縮む。

★★「widthは命令、MAXは天井、MINは床」★★

語呂の対応：
- 「width は命令」          → 固定値で必ずその幅にする
- 「MAX（max-width）は天井」 → 上限。それ以下は自由に縮む
- 「MIN（min-width）は床」    → 下限。それ以上は自由に広がる

【画面幅で見た動きの比較】
```
画面幅 1200px（PC）：
  width: 600px      → [   600px固定   ]   余白あり
  max-width: 600px  → [   600pxまで   ]   余白あり（同じに見える）

画面幅 400px（スマホ）：
  width: 600px      → [600px固定]はみ出す  → ❌横スクロール！
  max-width: 600px  → [400pxに縮む]         → ✅画面に収まる
```
...
.box {
  width: 100%;       /* 親に合わせて広がる */
  max-width: 600px;  /* でも 600px が上限 */
}
```

【補足】
- ⚠ 文章ブロックは基本 `max-width` を使う（読みやすさを保ちつつ画面に対応）
- ⚠ `width: 100% + max-width` のセットが王道（柔軟＋上限）
- 💡 画像には `max-width: 100%` を付けると親からはみ出さない（必須テク）
- 💡 三兄弟：`width`（固定） / `max-width`（上限） / `min-width`（下限）
- 【関連】→「コンテナの幅指定は width:100% + max-width + margin:0 auto」で検索（中央寄せもセットの形）

## 📌 width は固定値（必ずその幅）→ 画面が狭いとはみ出す。max-width は上限値（そこまでOK）→ 画面が狭ければ自動で縮む。max-width の目的は2つ：①スマホで崩れない（レスポンシブ）②PCで広がりすぎない（読みやすさ・60〜80文字で改行）。定番は「width: 100% + max-width: 値」のセット技 HTML CSS

【日付】2026-04-28

【結論】
- `width` = 固定指定（必ずその幅になる）→ 画面が狭くてもその幅を保とうとする → スマホで横スクロール発生
- `max-width` = 上限指定（そこまではOK）→ 画面が狭ければ画面幅に合わせて縮む → スマホでも崩れない

【比較表】

| プロパティ | 画面が広い時 | 画面が狭い時（400px） |
|---------|---------|---------|
| `width: 800px` | 800px | 800pxのままはみ出す ❌ |
| `max-width: 800px` | 800px | 画面幅に合わせて縮む ✅ |
...
```css
.box {
  width: 800px;  /* スマホ画面400pxでも800pxを保とうとする → 横スクロール発生 */
}
```

✅ width: 100% + max-width のセット技（定番）
```css
.box {
  max-width: 800px;  /* 上限 = PCでは800pxまで広がる */
  width: 100%;       /* 親の幅いっぱい = スマホでは画面に合わせて縮む */
}
```

【max-width の目的は2つある】
1. **スマホで崩れない**（レスポンシブ）→ 画面が狭ければ自動で縮むので横スクロールしない
2. **PCで広がりすぎない**（読みやすさ）→ 1行が60〜80文字で改行されるよう制限する。文章が画面いっぱいに広がると目線移動が大きくて読みにくくなるため

【補足】
- `width: 800px; max-width: 100%;` でも同じ動きをする（書き方が違うだけ）
- 文章ブロックや画像は基本 `max-width: 100%` か `width: 100% + max-width: 値` の組み合わせを使う
- レスポンシブ対応するなら原則 `width` より `max-width` を使う
- imgタグは `img { max-width: 100%; height: auto; }` が定番（はみ出し防止 + 縦横比維持）

## 📌 セクションレイアウトの設計指針 → ヒーローは「画像と並ぶ構造」で半分幅、他セクションは「テキスト中心」で全幅。役割が違うから幅違ってOK。新規セクションは position: absolute を避けて grid/flex で組むと、画像とテキストの重なり問題を根本から回避できる HTML CSS

【日付】2026-04-28

【結論】
- ヒーロー（hero）：画像と並ぶ構造 → テキストは半分幅で自然
- 他セクション：テキスト中心 → 全幅で広がる
- セクションごとに幅が違うのは「役割が違うから」で違和感ではない（多くのサイトの慣習）
- 新規セクションは position: absolute より grid / flex を選ぶ → 重なり問題が起きない

【具体例】

❌ position: absolute で画像配置（重なりやすい）
```css
.hero {
  position: relative;
}
...
.hero_text {
  max-width: 50%;  /* ← 手動で重なり対策が必要・ハマりポイント */
}
```

✅ grid で2カラムに並べる（重なり問題なし）
```css
.hero {
  display: grid;
...
【補足】
- ヒーローと他セクションで幅が違うのは多くのサイトで見られる慣習なので違和感を抱える必要はない
- 一貫性を高めたいなら全セクションに共通の `--container` 変数で max-width を統一する手もある
- `position: absolute` は「上下に重ねたい時」「装飾を浮かせたい時」だけ使うのが安全
- grid なら 2列・3列レイアウトを**論理的に保証**してくれる → レスポンシブも `grid-template-columns: 1fr` に切り替えるだけで縦並びにできる
- flex でも同じことができるが、2列固定なら grid の方が直感的
## 📌 border-radius の4値は時計回り（左上→右上→右下→左下）→ 2番目が右上 HTML CSS

【日付】2026-04-28

【結論】
`border-radius` に4つ値を書くと、時計回りの順番で角に対応する。
...
【補足】
- 値が大きいほど丸みの半径が大きくなる（アーチが高くなる）
- 目安：`8px`＝少し丸い／`30px`＝やわらかい／`100px〜200px`＝アーチ形
- 親要素に `overflow: hidden` がないと、子要素の角丸が背景色でつぶれて見える

## 📌 mix-blend-mode: multiply → PNG画像の白い背景をカード背景に馴染ませる（白が消えて見える） HTML CSS

【日付】2026-04-28

【結論】
`mix-blend-mode: multiply` を img に指定すると、画像の白い部分が親の背景色と合成されて消えたように見える。

【仕組み】
multiply（乗算）は 白 × 背景色 = 背景色 になる計算。
→ 白いピクセルだけが背景色に溶け込む（透過ではなく色の計算）
...
```
背景が薄緑のカードにPNGアイコンを置いたとき、アイコン周囲の白い四角が消えてカード色に馴染む。
...
## 📌 backdrop-filter: blur() → 要素の背後をすりガラス風にぼかす。background: rgba() と必ずセットで使う HTML CSS

【日付】2026-04-28

**結論**
`backdrop-filter` は要素自体ではなく、要素の**うしろ（背景）**にエフェクトをかけるプロパティ。

**よく使うセット**
```css
background: rgba(255, 255, 255, 0.2); /* 半透明にしないと透けない */
backdrop-filter: blur(12px);           /* うしろをぼかす */
```

**注意点**
- `background` が完全不透明だと後ろが透けないので効果が見えない
- `filter` → 要素自体にエフェクト
- `backdrop-filter` → 要素の**うしろ**にエフェクト

---

## 📌 rgba(r, g, b, a) の a = 透明度（0〜1）。#ffffff では透明度指定できないので透かしたいときは rgba に切り替える HTML CSS

【日付】2026-04-28

**結論**
`rgba` は色と透明度を同時に指定できる書き方。`rgb` に **a（alpha = 透明度）** を追加したもの。

```css
rgba(255, 255, 255, 0.85)  /* 白を85%不透明（15%透明） */
rgba(0, 0, 0, 0.4)         /* 黒を40%不透明（60%透明） */
rgba(42, 122, 102, 0.75)   /* 緑を75%不透明 */
```

- `0` = 完全透明 / `1` = 完全不透明
- `#ffffff` には透明度を指定できない → 透かしたいときは必ず `rgba`
- `opacity` はその要素の中身ごと透明になるのに対し、`rgba` は背景色だけ透明にできる

---

## 📌 background: グラデーション, url() → 画像の上にグラデーションをかぶせる複数背景テクニック。前に書いたほうが上に重なる HTML CSS

【日付】2026-04-28

**結論**
`background` にカンマ区切りで複数の値を書くと、**前に書いたものが上に重なる**。
画像の上にグラデーションをかぶせるときに使う。

```css
background:
  linear-gradient(135deg, rgba(42, 122, 102, 0.75) 0%, rgba(14, 59, 52, 0.75) 100%),
  url('../img/photo.png') center / cover no-repeat;
/* グラデーション（前）が上、画像（後）が下 */
```

**グラデーションの透明度で写真の見え方を調整**
- `0.85` → ほぼ色のみ（写真ほぼ見えない）
- `0.75` → 程よく写真が透ける（バランスが良い）
- `0.65` → 写真が目立つ（テキストが読みにくくなることも）

---

## 📌 display: grid + gap だけで縦並べもできる → grid-template-columns がなければ「1列縦積み」。flex-direction: column + gap と同等 HTML CSS

【日付】2026-04-28

**結論**
`display: grid` に `grid-template-columns` を書かなければ、子要素が **1列で縦に積まれる**。
子要素間の隙間（gap）をつけたいだけのときに使う。

```css
.timeline {
  display: grid;
  gap: 1.4rem; /* 子要素の間に均等な隙間 */
}
/* flex-direction: column + gap と同じ見た目になる */
```
...
## 📌 translate(-50%, -50%) は top: 50%; left: 50% とセットで使う中央配置専用テクニック → 位置が変わったら不要 HTML CSS

【日付】2026-04-30

【結論】
`translate(-50%, -50%)` は「top/left 50% で生まれるズレを補正する」ためだけに存在する。
基準点が50%以外に変わったら、このプロパティは不要。

【仕組み】
```
top: 50%; left: 50%;
```
→ 要素の**左上の角**が親の中心に来るだけ。要素の中心ではない。

```
transform: translate(-50%, -50%);
```
→ 自分の幅・高さの半分だけ左上にずらす → 要素の**中心**が親の中心に重なる。

【図解】
```
top/left: 50% だけ          + translate(-50%, -50%)
...
【いつ不要か】
- `top: 20%; left: 12%` のように任意の位置に置くとき → `translate` は使わない
- `position: static` や flex/grid の中央寄せを使うとき → `translate` 不要

【補足】
- `translate(-50%, -50%)` の `-50%` は**自分自身のサイズの50%**を基準にする（親ではない）

## 📌 動画ファイル自体に黒帯が入っているとき → object-fit: cover では消えない。transform: scale() で拡大して黒帯を見えなくする HTML CSS

【日付】2026-04-30

【結論】
`object-fit: cover` は動画の「表示方法」を変えるだけで、ファイルに焼き込まれた黒帯は消せない。
`transform: scale()` で動画を少し拡大して黒帯を画面外にはみ出させるのが CSS での応急処置。
...
object-fit: cover はこの「動画全体」を拡大するので、
黒帯ごとスケールアップ → 端に黒が残る
```
...
.home_hero_bg_video {
  ...
  height: 100%;
  object-fit: cover;
  transform: scale(1.08); /* ← 少し拡大して黒帯を外に追い出す */
}
```
- 親要素に `overflow: hidden` が付いていれば、はみ出た黒帯は自動で切れる
- 数値は黒帯の太さに合わせて調整（1.05〜1.15 あたりで試す）

【根本解決】
動画ファイルを書き出し直すとき「16:9・黒帯なし」で書き出す。
ChatGPT生成動画 → 再ダウンロードか再生成
自撮り動画 → CapCut・DaVinci Resolve などで書き出し直し
...
## 📌 スクリーンリーダーは画面を音声で読み上げる →
aria-hidden="true" で装飾要素（動画・アイコン）を読み上げから除外できる HTML

【日付】2026-05-01
【結論】
画面が見えない人が使うソフト。HTMLのテキストや要素の存在を上から順に読み上げる。意味のある要素はaltで説明し、装飾はaria-hidden="true"で黙らせる。

【具体例】
<img src="logo.png" alt="会社ロゴ">              ← 意味あり → alt必須
<video autoplay muted loop aria-hidden="true">    ← 装飾動画 → スキップ
<i class="fas fa-arrow" aria-hidden="true"></i>  ← アイコン → スキップ

| 属性 | 役割 |
|------|------|
| alt="説明" | 画像の内容を伝える |
| alt="" | 装飾画像としてスキップ |
| aria-hidden="true" | 要素ごとスキップ |
| aria-label="..." | ボタン等に説明文追加 |

【補足】
- aria- で始まる属性は「スクリーンリーダー向けの情報」と覚える
- 動画・アイコンフォント・装飾SVGには aria-hidden="true" を必ずつける

---

## 📌 動画の自動再生に必要な3点セット →
autoplay muted playsinline を揃えないとブラウザやiPhoneでブロックされる HTML

【日付】2026-05-01
【結論】
autoplay だけでは音ありの動画はブラウザにブロックされる。muted で音を消すと許可される。playsinline がないとiPhoneで全画面になる。
...
【補足】
- この3つはセットで書くのが定石
- 1つでも欠けると環境によって動かないことがある
- loop → 繰り返し再生（背景動画に必須）

---

## 📌 min(最大幅, 画面幅-余白) はレスポンシブの定番パターン →
大画面でコンテナが広がりすぎず、小画面で余白も保てる両立の書き方 HTML CSS

【日付】2026-05-01
【結論】
min() は2つの値のうち小さい方を使う CSS関数。コンテナの上限幅と画面余白の確保を1行で両立できる。

【具体例】
width: min(112rem, calc(100% - 12rem));
/*
  大画面（1400px以上）: 100%-12rem > 112rem → 112remで止まる
  小画面（800px以下）:  100%-12rem < 112rem → 両端に余白が残る
*/
...
【補足】
- var(--container) = 上限幅（例: 112rem）
- var(--gutter) * 2 = 左右余白の合計
- max-width + padding でも同じ意図だが、min() の方が1行で書ける

---

## 📌 ヒーロー上のヘッダーを透明にしてスクロールで帯を出す実装 →
.home_page .header を transparent にし、JSが header_scrolled クラスを付けて背景を復活させる HTML CSS

【日付】2026-05-01
【結論】
ヒーロー画像とヘッダーを一体化させるパターン。初期は透明、スクロールで帯が出る2段階の見た目。
...
.home_page .header.header_scrolled {
...
【補足】
- transition は CSS側に書く（スムーズに変化）
- body に home_page クラスを付けてページ限定スタイルにする
- 複数ページで同じヘッダーを使う場合、ページごとにbodyクラスで分岐する

---

## 📌 Web用画像を一括圧縮できるツール → TinyPNG HTML

【日付】2026-05-02
【結論】
TinyPNG は PNG・JPG を最大20枚まとめてドラッグするだけで圧縮できる無料Webツール。ファイル名はそのまま、見た目もほぼ変わらない。
...
1. img フォルダを開く
2. 画像を全選択（Ctrl+A）
3. TinyPNG にドラッグ＆ドロップ
4.「Download all」でまとめてダウンロード
5. img フォルダに上書き保存

【補足】
- 1枚500KB以上あるなら圧縮推奨。理想は150〜200KB以下
- 印刷・入稿用途には使わない（Web専用）
- 20枚超える場合は2回に分ける
...
## 📌 IntersectionObserver の threshold が高いと要素が見える前にアニメーションが完了してしまう → 0.1 にすると画面に入った瞬間に発火して自然 HTML CSS

【日付】2026-05-04

【結論】
`threshold: 0.4` は「40% が見えたら発火」。要素が縦に長いと、上半分が見えた時点でアニメーションが始まり、下の方が画面に入る頃には終わっている。

【具体例】
```js
// ❌ 40%で発火 → 長い要素は下が見える前にアニメーション完了
{ threshold: 0.4 }
...
【補足】
- 要素が小さい（ボタン・アイコン）なら 0.4 でもOK
- 要素が縦に長い（タイムライン・カード一覧）なら 0.1 が自然

---

## 📌 レスポンシブのブレークポイントは 768px が一般的なスマホ上限 → 700px は少し狭め・デザインが崩れ始める幅に合わせる HTML CSS

【日付】2026-05-04

【結論】
768px が標準（iPad mini の横幅）。「この幅以下はスマホ扱い」の目安として最もよく使われる。

【よく使う値】
| 対象 | よく使われる値 |
|------|-------------|
| スマホ | 767px 以下 / 768px 以下 |
| タブレット | 1024px 以下 |
| PC | 1025px 以上 |

【補足】
- 絶対的な決まりはない
- 「デザインが崩れ始める幅」に合わせるのが正しいアプローチ
- 700px でも崩れ始めるのがそこなら 700px でOK

---

## 📌 calc(10 / 375 * 100vw) → スマホで rem を画面幅に比例してスケールさせるパターン。375 = iPhone の基準幅 HTML CSS

【日付】2026-05-04

【結論】
`html { font-size: calc(10 / 375 * 100vw); }` と書くと、375px（iPhone）の画面で `1rem = 10px` になり、画面幅に比例して rem サイズが自動で拡縮する。
...
/* 375px の画面 → 10px（1rem = 10px）
   300px の画面 → 8px（自動で縮む）
   420px の画面 → 11.2px（自動で拡大） */
```

【補足】
- 375px は iPhone 6/7/8/SE の画面幅。スマホデザインの基準として最も使われる
- メディアクエリのスマホ用ブロック内に書くのが基本
- デザインカンプが 375px 幅で作られているなら、この値はそのままでOK

## 📌 object-fit: cover は「切り方」、object-position は「どこを見せるか」→ セットで使う HTML CSS

【日付】2026-05-04

`object-fit: cover` だけでは「はみ出しても黒帯を出さない」という切り方のルールしか決まらない。
デフォルトは中央（center center）を表示するが、人物が右寄りにいる動画・画像では重要な部分が切れてしまう。
`object-position` を追加することで「どこを中心に切るか」を指定できる。

```css
.hero_bg_video {
  object-fit: cover;             /* 黒帯なし・はみ出してカバー */
  object-position: right center; /* 右寄りに表示（人物が右にいる場合） */
}
```

| 値 | 意味 |
|----|------|
| `center center` | 中央（デフォルト）省略可 |
| `right center` | 右寄りに表示 |
| `left center` | 左寄りに表示 |
| `60% center` | 左から60%の位置を中心に |
...
### 🔧 2026-05-15 追加・修正した仕様

#### ① CSSヒント（VS Code右側のグレー文字）
- **PC定義 + @media定義の混在 → 「2箇所に定義」警告を出さない**
  - 意図的なスマホ上書きは重複とみなさない
  - 両方とも @media 内に定義 → 警告あり（本当の重複）
- **疑似要素ヒント追加**
  - ベースセレクタ（`.co_title`）の行に `[参考] 疑似要素あり（::before）` を表示
  - 疑似要素側（`::before`）には出さない（本クラスがあるのは当然のため）

#### ② PC/SPモード切り替えボタン（ステータスバー右下）
- `🖥 PC`（白）↔ `📱 SP`（オレンジ背景）でクリック切り替え
- **PCモード**：Ctrl+Click → @media外の定義（PC CSS）を優先してジャンプ
- **SPモード**：Ctrl+Click → @media内の定義を優先してジャンプ
- PCにしか定義がなければSPモードでもPC定義にフォールバック

#### ③ Ctrl+Click（PHPからCSSへのジャンプ）改善
- **疑似クラス（`:first-child` 等）がヒットしないよう修正**
- **空のスケルトンルール（プロパティなし）をスキップ**
- **標準CSSフォルダを自動優先**：`css/` `styles/` `assets/` 等 → 最優先、その他フォルダ → 後回し
- 除外フォルダ：`bkup/` `backup/` `node_modules/` `wp-includes/` `wp-admin/` `plugins/` 等

#### ④ 原因判明
- **CSS Navigation拡張機能**（by pucelle）が複数定義を返していた → 無効化で解決


## GMKtec paypal 異議申し立て　返金　ミニPC

・PayPalからーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
5月14日(木) 18:13

「4月のアカウント明細です」メール　明細をみると

5月9日　支払い・返金済み　-119,999JPY
5月12日　返金　+ 11,999JPY
GMKtec 

詳細画面
![](images/2026-05-15-16-05-10.png)

PC代金
![](images/2026-05-15-16-02-38.png)


------------------------------------------------------------------------
・5/15日　返金がクレジット、通帳に振り込まれていなことを確認
➡　電話して　山之内さんに　異議申し立てをおねがいする
------------------------------------------------------------------------


・GMKJP　2590
ご注文内容の確認　注文番号：#GMKJP2596 (ご注文内容の確認　注文番号：#GMKJP2596
のやりとり

メール
・ご注文頂きありがとうございました。　5月9日　10:28


・「返信」5月9日　10:49
![](images/2026-05-15-16-09-32.png)

GMKtec JP ご担当者様

お世話になっております。

注文番号 #GMKJP2596 の注文について、キャンセルをお願いいたします。

商品：
GMKtec K8 Plus AMD Ryzen 7 8845HS ミニPC
32GB RAM + 1TB SSD

金額：
119,999円

まだ発送前でしたら、発送停止とキャンセル、返金手続きをお願いいたします。
...
やはり、注文番号 #GMKJP2596 の注文について、キャンセルをお願いいたします！！
...
まだ発送前でしたら、発送停止とキャンセル、「早急」に返金手続きをお願いいたします。

よろしくお願いいたします。


・「返信」　5月9日　18:23
Hello, your email is blank. Please check it as soon as possible to see if you should proceed with the order.
Best Regards!

Yan

「こんにちは。お客様のメールが空白です。ご注文を進めても良いか、できるだけ早くご確認ください。」

Customer Service Team | GMKtec Official Store
![](images/2026-05-15-16-11-23.png)



・GMKtec JP ご担当者様　5/9 18:35

お世話になっております。
注文番号 #GMKJP2596 につきまして、キャンセルをお願いいたしたくご連絡いたしました。

・注文番号:#GMKJP2596
・商品:GMKtec K8 Plus AMD Ryzen 7 8845HS ミニPC(32GB RAM + 1TB SSD)
・金額:119,999円

まだ発送前であれば、発送停止・キャンセル・全額返金の手続きをお願いいたします。

お手数ですが、2営業日以内を目安に、以下3点をご返信いただけますと幸いです。

1. キャンセル受付の可否
2. 返金方法(決済時と同じカードへの返金という理解で違いないか)
3. 返金完了までの見込み期間
...
![](images/2026-05-15-16-29-48.png)


・「返信」　5月9日　19:01

Dear, we have cancelled your order. The refund is expected to be completed within one week. We look forward to having the opportunity to work with you again in the future.

「お客様、ご注文をキャンセルさせていただきました。返金は1週間以内に完了する予定です。今後またお取引できる機会を心よりお待ちしております。」


![](images/2026-05-15-16-27-03.png)


-------------------------------------------------------
・別メール　tec⇀おれ　5月12日　11:17
「返金通知」
...
注文状況
	GMKtec K8 Plus AMD Ryzen™ 7 8845HS ミニPC × 1
AMD Ryzen 7 8845HS / 32GB RAM+1TB SSD / 1年保証付き	
¥119,999
...
![](images/2026-05-15-16-15-44.png)



・別メール　おれ⇀tec　5月15日　8:47
GMKtec JP ご担当者様

注文番号：GMKJP2596 について確認です。

注文合計は119,999円ですが、PayPal上では「返金された合計額：11,999円」と表示されています。

全額返金のはずですが、差額108,000円が返金されていない可能性があります。

返金処理が119,999円で行われているか確認し、もし11,999円のみの返金処理になっている場合は、残り108,000円の
「早急」に返金対応をお願いします。

しかもまだ、銀行に入金されていません。

![](images/2026-05-15-16-16-57.png)


・別メール　tec⇀おれ　5月15日　12:16
Dear, the remaining money will be returned to you as soon as possible. Thank you.

Best Regards!

Yan

「お客様へ、残りの金額はできるだけ早く返金いたします。ありがとうございます。

よろしくお願いいたします。「

Customer Service Team | GMKtec Official Store

Global Site: www.gmktec.com

EU Site: de.gmktec.com

JP Site: jp.gmktec.com

UK Site: www.gmktec.uk

![](images/2026-05-15-16-19-12.png)


・別メール　おれ⇀tec　5月15日　15:20
Yan様

返信ありがとうございます。  

注文金額は119,999円ですが、PayPal上では11,999円のみ返金表示となっております。

また、現時点ではその11,999円についても、カード・口座側で返金反映を確認できておりません。

残り108,000円を含め、合計119,999円の返金処理状況、処理日、返金金額をご確認のうえ、早急に、ご連絡ください。

よろしくお願いいたします。

![](images/2026-05-15-16-20-19.png)

## claude codeへの指示方法　AIの活用方法　プライベートで意識すること

【設計フェーズ】
・claudeに考える時間をあたえる（「これ、自分で答えを出してから AI に確かめる順番にできないか?」と自問してみてほしい）
　⇀　～この機能を追加しようと思うけど「どう思う？」と聞く癖　人の設計思想をこえているわけだら
　⇀　～この機能がなければどうなる？　⇀不必要ななきのがだせる

【プライベート】
　⇀　すぐネットできくようにさせる。AIは独自の考えになりがち、一度検索させるというひと手間を与えると精度がたかまる
　⇀　いちばん手戻りが大きい工程に Claude を投入する（設計・）
　⇀　「明日、最初に着手する仕事で、いきなり手を動かさず、まず Claude にどう問うか」を 1〜2行で決めておくこと。これだけ。

【AIの利用方法】
　⇀　 Claude.ai のメモリ機能と user preferences。
　⇀　、「自分のこと」と「プロジェクトのこと」は別レイヤーで管理する
　⇀　ただしたまに見直して、古い情報や誤解は消す。Claude のメモリ設定からは、自分が登録されている内容を見られるし、削除もできる。
　⇀　月に1回くらい設定画面を開いて見直す
...
**旧PC** → **新SSD（250GB）に一時保管** → **新PC** という流れ。
新SSDが「引っ越しトラック」の役割を果たす。

---

### 📂 新SSD（250GB）のフォルダ構成

新SSDにこの構造でバックアップを保存する：

`F:\（新SSD 250GB）
│
├─ 01_WSL\
│   └─ wsl-backup.tar
│       → WSL環境（Claude Code・Codex・Python・git設定・APIキー等
│         全部入り）を1ファイルにまとめたもの
│
├─ 02_ユーザー設定\
│   └─ sensha\
│       ├─ .claude\         ← Claude Codeの認証・設定
│       ├─ .codex\          ← ChatGPT Codexの認証・設定
│       ├─ .cursor\         ← Cursorの設定
│       ├─ .copilot\        ← GitHub Copilotの認証
│       ├─ .gemini\         ← Geminiの認証
│       ├─ .config\         ← 各種ツールの汎用設定
│       ├─ .docker\         ← Docker設定
│       ├─ .gitconfig       ← Git設定
...
│
├─ 03_Dドライブ\
│   └─ （Dドライブの中身を厳選してコピー、230GB以下に）
│       ├─ 01_AI\
│       ├─ 02_作業\
│       ├─ 05_画像\
│       ├─ Programs\（ポータブル版ソフトのみ）
│       ├─ 50_knowledge\
│       └─ その他必要なフォルダ
│
├─ 04_スタートアップ\
│   └─ （shell:startup の中身、AHKショートカット等）
│
└─ 05_メモ\
    ├─ 再インストールソフトリスト.txt
    ├─ APIキー類の場所メモ.txt
...
- パーティション分割はしない（フォルダ分けで十分）
- `02_ユーザー設定` には認証情報が入るので、移行完了後は削除する

---

### 🟦 フェーズ1：旧PCで準備（PC到着前にやる）
...
→ できたtarファイルを新SSDの`01_WSL\`に保存

- [ ]  **ユーザーフォルダのドット始まりフォルダをコピー**
→ `C:\Users\sensha\` の隠しフォルダ表示をONにして、ドット始まりフォルダ全部を新SSDの`02_ユーザー設定\sensha\`にコピー
- [ ]  **スタートアップフォルダをコピー**
→ エクスプローラーのアドレスバーに `shell:startup` と入力 → 中身を新SSDの`04_スタートアップ\`にコピー

### 同期系の確認

- [ ]  **VS Code Settings Sync 有効確認**
→ コマンドパレットで「Settings Sync」と入力。「オフにする」が表示されてれば有効中。新PCでログインするだけで設定・拡張機能が復元される。
- [ ]  **Chrome Googleアカウント同期確認**
→ 設定 → Google同期がオンか確認。ブックマーク・拡張機能・パスワードが自動同期される。
...
- [ ]  **不要データを削除**
    - CapCutキャッシュ
    - 古いISOファイル（CentOS等）
    - 過去のcommit zip（GitHubにあるなら）
    - 完成済み動画素材
    - その他「いつか使うかも」ファイル
- [ ]  **`D:\Programs\` のインストール型ソフトを削除**
→ コピーしても新PCで動かないので削除。ポータブル版（ShareX・WinDirStat・TCPView等）はフォルダごと残す。
- [ ]  **整理後のDドライブを新SSDの`03_Dドライブ\`にコピー**
→ 230GB以下に収まってることを確認

### メモ作成

- [ ]  **再インストールソフトリストを作成**
→ Notionに「PCセットアップチェックリスト」ページ作成。新SSDの`05_メモ\`にもテキストで保存。
例：
...
- [ ]  **CrystalDiskInfoで1TB SSDの健康状態確認**
→ 「正常」表示、使用時間が極端に長くないか確認
- [ ]  **HWiNFO64で温度・パーツ認識確認**
→ アイドル時CPU/GPU 40〜50℃が正常。スペック表通りのパーツが認識されてるかチェック。
- [ ]  **3DMarkかFurMarkで軽くGPUベンチマーク**
→ RTX 3070 Tiの標準スコアと比較して、明らかに低くないか確認
- [ ]  **★ここまで確認できたらメルカリ受け取り評価**
→ 評価前ならトラブル時に交渉できるので、動作確認まで終わらせてから評価する
...
powershell
  ...
  `wsl --import Ubuntu C:\WSL\Ubuntu F:\01_WSL\wsl-backup.tar`

- [ ]  **WSL起動 → Claude Code・Codex動作確認**
→ これでWSL内の環境は完全復元される

### ステップ2：★ドットファイルを先に配置

- [ ]  **新SSDの`02_ユーザー設定\sensha\`の中身を`C:\Users\sensha\`にコピー**
→ `.claude`、`.codex`、`.cursor`、`.copilot`、`.gemini` などを所定の場所に
→ **★まだソフトをインストールしないなぜこの順序か**：ソフトをインストール・起動すると新規設定フォルダが作られる。後から上書きすると認証情報が壊れたり、動作がおかしくなる可能性がある。**先にバックアップを置いておけば、ソフトは「設定あるじゃん」と認識して既存のものを使う。**

### ステップ3：ソフトをインストール（★起動しない）

- [ ]  Chrome（インストールのみ）
- [ ]  VS Code（インストールのみ）
- [ ]  Claude Desktop（インストールのみ）
- [ ]  Cursor（インストールのみ）
- [ ]  Obsidian（インストールのみ）
- [ ]  AutoHotkey
- [ ]  その他、Notionリストのソフト

→ **インストール後、起動はせずに次のステップへ**

### ステップ4：初回起動して動作確認

- [ ]  **Chrome起動** → Googleログイン → 同期復元確認
- [ ]  **VS Code起動** → Settings Syncログイン → 拡張機能・設定復元確認
- [ ]  **Claude Desktop起動** → ログイン状態確認
- [ ]  **Cursor起動** → ログイン状態確認
- [ ]  **Obsidian起動** → Vault指定（Dドライブに復元したVaultフォルダを指定）
...
- [ ]  **Dドライブの中身を新SSDから復元**
→ 旧PCと同じパス構造で配置する（例：`D:\02_作業\`、`D:\Programs\` など）
→ スタートアップのショートカット切れを防ぐため、パスは厳密に揃える
- [ ]  **スタートアップフォルダにショートカット配置**
→ `shell:startup` で開く場所に、新SSDの`04_スタートアップ\`の中身を貼り付け
- [ ]  **CSS Jumper等の自作Chrome拡張を読み込み**
→ `chrome://extensions/` → デベロッパーモードON → 「パッケージ化されていない拡張機能を読み込む」→ Dドライブのソースコードフォルダを指定
- [ ]  **AHKスクリプト動作確認**
→ ショートカットからスクリプトが起動するか確認
...
- [ ]  **1日普通に作業してみる**
→ 実際に仕事してみて違和感がないか確認
- [ ]  **★新SSDから認証フォルダを削除（安全対策）**
→ `02_ユーザー設定\sensha\` の中の `.claude`、`.codex`、`.gemini`、`.copilot` などを削除
→ 認証情報入りSSDを持ち歩くのはリスクのため
- [ ]  **新SSDを持ち運び用に戻す**
→ 通常のデータ運搬・バックアップ用途で使える状態に
- [ ]  **1〜2週間問題なければ旧PC整理**
→ データ完全消去 or 譲渡先決定
...
1. **WSL import**（Claude Code・Codexはこれで復元）
2. **ドットファイル配置**
3. **Chrome + VS Code + Claude Desktop インストール**
4. **動作確認**

これで半日以内に仕事再開できる。他のツールは必要になった時に都度入れればOK。
...
これをNotionにそのままコピペすれば、チェックボックスも生きた状態で使えます。

PC到着まで、まずはフェーズ1から進めていきましょう！ 詰まったら気軽に聞いてください 🚀
...
├─ 01_WSL\
│   └─ wsl-backup.tar
│       → WSL環境(Claude Code・Codex・Python・git設定・APIキー等全部入り)
│
├─ 02_ユーザー設定\
│   └─ sensha\
│       ├─ .claude\         ← Claude Codeの認証・設定
...
└─ 05_メモ\
    ├─ 再インストールソフトリスト.txt
    ├─ env_user.txt        ← Windowsユーザー環境変数
    ├─ env_system.txt      ← Windowsシステム環境変数
    ├─ APIキー類の場所メモ.txt
    └─ 復元手順メモ.md`
...
- パーティション分割はしない（フォルダ分けで十分）
- `02_ユーザー設定` には認証情報が入るので、移行完了後は削除する
- 環境変数のテキストにAPIキーが含まれてれば削除して別管理（Notion）
...
powershell
  ...
  `[Environment]::GetEnvironmentVariables("User") | Format-List > F:\05_メモ\env_user.txt
  [Environment]::GetEnvironmentVariables("Machine") | Format-List > F:\05_メモ\env_system.txt`

→ ファイルを開いて、APIキーが含まれてれば削除（Notionから別途復元）
...
- [ ]  **「見えない設定」の確認とメモ**
    - IME辞書のエクスポート（Microsoft IME → 設定 → 辞書のエクスポート）
    - タスクスケジューラの定期実行タスクをメモ（あれば）
    - ファイル関連付けの特殊設定をメモ（あれば）
...
### 🟨 フェーズ3：Windows初期設定（★最初の30分が肝心）

### ★Windowsセットアップ（一番最初にやる）

- [ ]  **ネット切断状態でセットアップ開始**
→ LANケーブル抜く、Wi-Fi繋がない。これでローカルアカウント作成画面に進める。
- [ ]  **★ローカルアカウント作成、ユーザー名は「sensha」**
→ 旧PCと完全に同じユーザー名にする。これを間違えると後で全部のパスがズレる。
- [ ]  **セットアップ完了後、ネット接続**
- [ ]  **必要ならMicrosoftアカウントを後から紐付け**

### ★パーティション分割（データを入れる前にやる）

- [ ]  **「ディスクの管理」を開く**
→ スタート右クリック → 「ディスクの管理」
- [ ]  **Cドライブを縮小**
→ Cドライブ右クリック → 「ボリュームの縮小」→ 600〜700GB分を縮小
→ Cドライブが約300〜400GBになる
- [ ]  **空き領域からDドライブを作成**
→ 空き領域を右クリック → 「新しいシンプルボリューム」
→ ドライブ文字「D」を割り当て → NTFSでフォーマット
...
- [ ]  **新SSDの`05_メモ\env_user.txt`を見ながら環境変数を再設定**
→ `sysdm.cpl` → 詳細設定 → 環境変数 で手動入力
→ または PowerShellで一括設定：
...
- [ ]  **APIキーはNotionから手動入力**
→ 平文でテキストに残さない
- [ ]  **PATHに自作ツールパスを追加**（必要に応じて）
- [ ]  **サインアウト→再ログイン**で環境変数を反映

### ステップ4：ソフトをインストール（★起動しない）
...
1. **Windowsセットアップ（アカウント名「sensha」）**
2. **パーティション分割（C/D作成）**
3. **WSL import**（Claude Code・Codexはこれで復元）
4. **ドットファイル配置**
5. **環境変数の最低限の復元**（APIキーなど）
6. **Chrome + VS Code + Claude Desktop インストール**
7. **動作確認**
...
1. **★ユーザー名は「sensha」**（旧PCと完全に同じ）
2. **★パーティション分割はデータを入れる前**
3. **★ドットファイルはソフトインストール前に配置**
4. **★環境変数のAPIキーは平文で持ち運ばない**

この4つを最初に押さえれば、後はチェックリスト通りで完了します。