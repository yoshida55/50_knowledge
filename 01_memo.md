...
🔍 parmalink
━━━━━━━━━━━━━━━━━━━━━━━━━━
📌 何をする  パーマリンク（固定リンク）を取得する
📥 引数      なし
📤 戻り値    文字列 → "https://example.com/my-post/"
            └ 取得後: リンクとして表示
💡 使い方    `const link = document.location.href;`
            `window.open(link);`
⚠️  注意      動的なページでは注意
            └ サーバー側で生成される場合も
━━━━━━━━━━━━━━━━━━━━━━━━━━
...
/* ✨
固定リンク（パーマリンク）とは、**特定のWebページや記事に対して割り当てられた、永続的で変わることのないURL**のことです。

一度設定すると内容を変更してもURLが変わらないため、ブックマークやSNSでのシェア、他のサイトからのリンクに適しています。
*/

/* ✨
パーマリンクは、主に**ブラウザの現在のアドレスバー（`document.location.href`）**から取得します。

ただし、Webサイトの仕組みによっては、サーバー側で生成されたURLをHTMLのメタタグや属性から読み取る場合もあります。
*/

/* ✨
結論から言うと、**「スラッグ」はパーマリンクの一部**です。

パーマリンクが「URL全体（例：`https://example.com/blog/my-post`）」を指すのに対し、スラッグは「その末尾の識別部分（例：`my-post`）」を指します。

つまり、スラッグはパーマリンクを構成するパーツの一つという関係性です。
*/

パーマリンク（Permalink）とは「Permanent Link（恒久的なリンク）」の略で、Webサイトの各ページに対して割り当てられる、永続的で変更されないURLのことです。WordPressでは「設定」>「パーマリンク」からURLの構造（スラッグなど）をカスタマイズできます。


### permalink とは？
### permalink（パーマリンク）とは？
2026-03-08
Webサイトの各ページに割り当てられる「個別のURL」のことです。
「Permanent Link（恒久的なリンク）」の略で、一度決めたら後から変えないのが基本です。

*   **役割**: 誰がどこからアクセスしても、同じページにたどり着けるようにするための「住所」です。
*   **構成要素**: メインのドメイン ＋ **スラッグ**
    *   例: `https://example.com/about/`
    *   この場合、`/about/` の部分が「スラッグ」と呼ばれます。
*   **WordPressでの仕組み**:
    *   管理画面で設定した「スラッグ」がそのままURLの末尾になります。
    *   URL（パーマリンク）と、テーマ内のファイル名（`page-スラッグ.php`）を一致���せることで、特定のデザインをそのページに自動で適用できます。
*   **なぜ重要か**:
    *   分かりやすいURLにすると、検索エンジンや利用者が「何が書いてあるページか」を理解しやすくなります。
    *   一度公開した後にURLを変えると、それまでのリンクが壊れてしまうため、最初にしっかり決めることが大切です。

    

### パーマリンクとは？
パーマリンクとは、Webサイトの各ページに割り当てられた「個別のURL」のことです。

*   **URLの末尾部分**：ユーザーや検索エンジンが、どのページを見ているか判断するための目印になります。
*   **スラッグとの関係**：URLの末尾（スラッグ）を自分で決めることで、分かりやすいURLを作れます。
    *   例：`https://example.com/about/` （`about` がスラッグ）
*   **WordPressでの役割**：
    *   スラッグを設定すると、対応する名前のファイル（例：`page-about.php`）を自動で呼び出す仕組みがあります。
    *   これにより、「どのページにどのデザインを表示するか」を整理しやすくなります。
