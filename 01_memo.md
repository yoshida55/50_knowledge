...
## 🎯 JavaScriptの考えた・特定の要素までのスクロール位置の計算処理、（getBoundingClientRect、window.innerHeightのとは）スクロールするとごとにgetBoundingClientRectがへる。目的の要素をこえたらマイナスになる。
...
### DBから取得するものはescをつける
### なぜ「DB（データベース）から取得するもの」にescをつけるのか？
...
### 楽天モバイル
*   **特徴**: 楽天グループが運営する携帯電話サービス。
*   **料金**: 使ったデータ量に応じて月額料金が変わる仕組み。たくさん使っても上限があるため、安く抑えやすい。
*   **通信エリア**: 楽天独自の基地局に加え、auの回線も借りているため、全国で広く使える。
*   **メリット**:
    *   専用アプリ「Rakuten Link」を使えば、国内通話が無料。
    *   楽天ポイントが貯まったり、支払いに使えたりする。
    *   契約事務手数料や解約金がかからない。
*   **注意点**: 地下や山間部など、場所によってはつながりにくいことがある。
## クイズを解く際のルール
