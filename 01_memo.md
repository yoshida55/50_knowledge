...
/* ✨
`get_the_category()` は、**その投稿が属しているカテゴリー情報の「オブジェクトの配列」**を返します。

具体的には、以下のようなデータが入ったオブジェクトが、カテゴリーの数だけ配列として格納されます。

### 返り値のイメージ
Array (
    [0] => WP_Term Object (
        [term_id] => 5            // カテゴリーID
        [name] => お知らせ        // カテゴリー名（cat_name）
        [slug] => news            // スラッグ
        [term_group] => 0
        [term_taxonomy_id] => 5
        [taxonomy] => category
    ...
    [1] => WP_Term Object (...)   // 2つ目の
    カテゴリーがあれば続く
)

**ポイント：**
*   **配列（Array）**で返ってくるため、ご提示のコードのように `foreach` で回して取り出す必要があります。
★  `$category->cat_name` としている部分は、オブジェクトのプロパティである `name` を指しています（※`cat_name` は非推奨ですが、後方互換性のため動くようになっています。現在は `$category->name` と書くのが一般的です）。

★cat_name、name「name」はWordPress側で決まっている名前
$category->cat_name
★取り出したカテゴリオブジェクトの中の cat_name（name） という値を見る「配列ではなくて、オブジェクトである」

★今回の $category はカテゴリ情報を持つオブジェクトなので、[] ではなく -> を使っています。
配列の場合は、$array['name'];      // 配列
*/
...
### get_category_link() の本質 → IDかオブジェクトを渡すだけでURLが返ってくる

【日付】2026-03-16
【結論】
`get_category_link()` は「IDかオブジェクトを渡す → URLが返ってくる」それだけ。
URL は「サイトのベースURL + 管理画面で設定したスラッグ」をWordPressが自動で組み立てる。
...
【補足】
- ⚠ IDとオブジェクトのどちらを渡しても同じ結果（WordPressが内部でIDを取り出す）
- 💡 つまり：「IDかオブジェクト → URL」渡すだけ。URLを自分で書かなくていい

---


### archive.php　にカテゴリのだしかた
* **リンク付きでカテゴリーを表示するコード**
...
* **ポイント**
    * `get_category_link($category->term_id)`：カテゴリーごとのIDを使って、そのカテゴリー一覧ページへのURLを自動で作る関数。
    * `$category->name`：カテゴリーの名前を取り出す（`cat_name`より新しい書き方）。
    * `$category->term_id`：カテゴリーのID。これをリ���ク作成の材料にする。

### get_category_link()とは、わかりやすく、なにからなにを取得
### get_category_link() とは
カテゴリーの「URL」を取得するための命令です。

### 何から何を取得するか
*   **元（引数）:** カテゴリーのID（`$category->term_id` など）
*   **先（戻り値）:** そのカテゴリー専用ページへのリンク（URL）

### 使い方の例
`get_category_link()` だけではURLの文字列を取得するだけなので、HTMLの `<a>` タグと組み合わせて使います。
...
**ポイント**
*   `$category->term_id` を入れることで、WordPressが裏側で管理している「そのカテゴリーのページURL」を自動的に探し出して持ってきてくれます。

### get_category_link()とは、わかりやすく、なにからなにを取得する
* **何をするもの？**
  カテゴリー専用ページ（そのカテゴリーの記事が一覧で表示されるページ）へ移動するための「URL」を取得する関数です。

★ようするに、カテゴリに「スラッグ」をつけることはできるけど、「カテゴリの一覧」から選択したときに、そのスラッグに飛ぶURL（パーマリンク）ってことかな？

* **なにから取得する？**
  カテゴリーの「ID（識別番号）」から取得します。

* **コードの例**
  `get_category_link($category->term_id)` のように、カテゴリーのIDを渡して使います。

* **使い方（リンクを貼る場合）**
```php
<?php
  // カテゴリーのIDを渡すとURLが返ってくる
  $link = get_category_link($category->term_id);
...
### WordPressで画面で設定したカテゴリを取得するロジックは、
WordPressで画面で設定したカテゴリを取得するロジックは、以下の通りです。

*   **`get_the_category()` を使う**
    *   現在表示している記事に設定されたカテゴリ情報を、まとめて取得する命令です。
*   **`foreach` で一つずつ取り出す**
    *   カテゴリは複数設定できる可能性があるため、配列というリスト形式で返ってきます。そのため、ループを使って1つずつ処理します。
*   **名前やリンクを表示する**
    *   名前：`$category->name`（または `$category->cat_name`）
    *   リンク：`get_category_link($category->term_id)`
...
**ポイント：**
*   `$category->name`：設定したカテゴリの名前（文字）を取り出す。
*   `get_category_link($category->term_id)`：カテゴリごとのURLを自動的に作ってくれる。
*   `->`：データの中から特定の情報を取り出すための記号。

### 画面で取得したカテゴリ、つまり WordPress のカテゴリを取得するロジックは何でしょうか？
WordPressのカテゴリを取得するロジックは以下の通りです。

### 基本的な仕組み
1.  **情報の取得**: `get_the_category()` を使い、投稿に紐づくカテゴリ情報を「オブジェクトの配列」として取得する。
2.  **ループで取り出し**: 複数のカテゴリが設定されている可能性があるため、`foreach` を使って一つずつ順番に取り出す。
3.  **情報の表示**: 取り出したオブジェクトの中にある「名前（name）」や「ID」を使って画面に表示する。
...
foreach ($categories as $category) {
    // 3. リンク付きで表示する
    $link = get_category_link($category->term_id); // カテゴリのURLを取得
    echo '<li><a href="' . $link . '">' . $category->name . '</a></li>';
}
?>
```
...
### 押さえておくべきポイント
*   **`->`（アロー演算子）**: カテゴリは「オブジェクト」という形式でデータが入っているため、値を取り出すときは `[]` ではなく `->` を使います。
*   **`name` プロパティ**: カテゴリ名を表示する際は `$category->name` を使います（`cat_name` も動きますが、現在は `name` が推奨です）。
*   **リンクの作成**: `get_category_link()` にカテゴリのID（`term_id`）を渡すと、そのカテゴリ専用ページのURLを自動で作ってくれます。

### 画面で取得したカテゴリ、WordPressで取得するロジックは何？
WordPressで画面上のカテゴリを取得・表示するロジックは以下の通りです。

### 手順
1.  **情報の取得**: `get_the_category()` を使い、投稿に紐付く「カテゴリー情報の詰め合わせ（オブジェクトの配列）」を全て取得する。
2.  **繰り返しの処理**: カテゴリーは複数設定される可能性があるため、`foreach` を使って1つずつ取り出す。
3.  **表示**:
    *   名前：`$category->name`（または `$category->cat_name`）で取得。
    *   リンク：`get_category_link($category->term_id)` を使って、IDから自動的にURLを作成する。
...
### 重要なポイント
*   **配列とオブジェクト**: `get_the_category()` は「オブジェクトが入った配列」を返す。そのため、中身を取り出すときは `$array['key']` ではなく `$object->property` という書き方をする。
*   **名前の指定**: `$category->name` が現在の正式な書き方。
*   **URLの生成**: `get_category_link()` にカテゴリーIDを渡すだけで、WordPressが自動で正しいリンク先（スラッグに基づいたURL）を作ってくれる。

### WordPressで画面上で指定したカテゴリーを取得するロジックは何でしょうか？
WordPressで現在表示している記事のカテゴリーを取得するロジックは以下の通りです。

### 基本的な考え方
1.  **取得**: `get_the_category()` で、その記事が属するカテゴリー情報を「オブジェクトの配列」として全てまとめて手に入れる。
2.  **分解**: 配列なので `foreach` 文を使って、1つずつカテゴリー情報を取り出す。
3.  **表示**: 取り出したカテゴリー情報の「名前」や「リンク先URL」を表示する。
...
### ポイント
*   **`get_the_category()`**: 記事に設定されたカテゴリー情報を全部拾ってくる（表示はしない）。
*   **`foreach`**: カテゴリーが複数ある場合に、1つずつ処理するために使う。
*   **`$category->name`**: カテゴリー名を表示する。
*   **`get_category_link()`**: そのカテゴリー専用のページURLを自動で作ってくれる関数。引数にカテゴリーのID（`term_id`）を渡す必要がある。
*   **`->`（アロー演算子）**: カテゴリー情報の箱（オブジェクト）の中から特定のデータを取り出すための記号。
...
get_the_category()  → この記事の カテゴリー
get_the_terms()     → この記事の ターム
get_the_ID()        → この記事の ID
get_the_title()     → この記事の タイトル

なのでメモの「記事データ」という表現、ほぼ合ってます！
...
### `the_` がつくと記事限定 → ループの中でしか使えない

【日付】2026-03-16
【結論】
`the_` または `get_the_` がつく関数は**ループの中でしか正しく動かない**。
「どの記事か」が決まっていないと、WordPressが情報を引っ張れないため。
`get_` だけ（`the_` なし）はサイト全体のデータを取るので**ループ外でも使える**。

```
ループの中でしか使えない（記事限定）:
  get_the_category()  → この記事のカテゴリー
  get_the_terms()     → この記事のターム
  get_the_ID()        → この記事のID
  the_title()         → この記事のタイトルを表示

ループ外でも使える（サイト全体）:
  get_categories()    → サイト全カテゴリー
  get_category_link() → カテゴリーのURL
  get_theme_file_uri()→ テーマのファイルパス
```

【補足】
- ⚠ `the_` や `get_the_` をループ外で使っても何も出ない（エラーにはならず無視される）
- 💡 つまり：`the` がついていたら「この記事の〇〇」= ループ必須と覚える
...
WordPress関数の命名規約（黄金ルール）は以下の通りです。

* **`the_`（出す）**
  - そのまま画面に表示します。`echo`は不要です。
  - 例：`<?php the_title(); ?>`

* **`get_` / `get_the_`（取ってくる）**
  - データを持ってくるだけです。画面に表示するには`echo`が必要です。
  - 例：`<?php echo get_the_title(); ?>`

* **`has_` / `is_`（調べる）**
  - 「ある？」「〇〇ページ？」と判定します。`if`文の条件として使います。
  - 例：`<?php if (is_single()): ?>`

* **`add_`（登録する）**
  - WordPressに機能を追加します。主に`functions.php`で使います。
  - 例：`<?php add_theme_support('post-thumbnails'); ?>`
...
## WordPress カテゴリ関数4つの使い分け（the_ / get_ / wp_list_ / get_categories）
【日付】2026-03-16

【結論】
「その投稿のカテゴリ」か「全カテゴリ」か、「表示まで自動」か「取得だけ」かで4通りに分かれる。
`get_` 系はオブジェクトの配列で返ってくるので `foreach` + `->name` で取り出す。

| 関数 | 対象 | 動作 |
|---|---|---|
| `the_category()` | その投稿のカテゴリ | 表示まで自動（aタグ付き）|
| `get_the_category()` | その投稿のカテゴリ | オブジェクト配列で取得 |
| `wp_list_categories()` | 全カテゴリ | 表示まで自動（current-catクラス自動付与）|
| `get_categories()` | 全カテゴリ | オブジェクト配列で取得 |

【具体例】
```php
// get_the_category() の使い方（その投稿のカテゴリ）
$categories = get_the_category();  // オブジェクトの配列が返る
foreach ($categories as $category) {
    echo esc_html($category->name);  // ->name でカテゴリ名を取り出す
}

// get_categories() の使い方（全カテゴリ）
...
// wp_list_categories() の使い方（全カテゴリ・自動出力）
wp_list_categories(array(
    'title_li' => ''  // 見出しを非表示にする
));
// → <li><a href="...">カテゴリ名</a></li> が自動で出力される
// → 選択中のカテゴリのliに current-cat クラスが自動でつく
```

【補足】
- ⚠ `the_` と `wp_list_` は表示まで自動なのでechoは不要。`get_` はechoが必要
- ⚠ `get_` 系の戻り値は「オブジェクトの配列」→ `->name` で取り出す（`['name']` ではない）
- 💡 つまり：自動表示でいいなら `the_` / `wp_list_`、HTMLを自由に作りたいなら `get_` を使う

### get_categories() の注意点3つ
【日付】2026-03-16

- ① **サイト全体**のカテゴリを取得する（`get_the_category()` はその投稿のカテゴリだけ）
- ② 投稿に紐づいていないカテゴリ（投稿0件）はデフォルトで**非表示**
- ③ 全部表示したいときは `hide_empty => false` を渡す
...
```
get_the_category() → その投稿についているカテゴリだけ
get_categories()   → サイト全体のカテゴリ全部
```

- ⚠ カテゴリを作っても投稿と紐づけないと0件 → デフォルトでは表示されない
- 💡 つまり：カテゴリ絞り込みボタンなど「全カテゴリ一覧」を出すときに使う
【関連】→ 「オブジェクト 配列」で検索（-> と [] の使い分け詳細）


### カテゴリ関連PHP関数4種（the_category / get_the_category / wp_list_categories / get_categories）
WordPressで、ブログ記事の分類（カテゴリ）を表示したり取得したりするための命令です。用途に合わせて以下の4種類を使い分けます。

### 関数一覧と使い分け
| 関数名 | 対象 | 動作 |
| :--- | :--- | :--- |
| `the_category()` | その投稿のカテゴリ | 自動でHTML（リンク）出力 |
| `get_the_category()` | その投稿のカテゴリ | データだけ取得（加工用） |
| `wp_list_categories()` | 全カテゴリ | リスト形式で自動出力 |
| `get_categories()` | 全カテゴリ | データだけ取得（加工用） |

### 覚えておくべきルール
*   **自動表示（`the_`, `wp_list_`）**: `echo` 不要。そのまま書くだけで画面に出る。
*   **データ取得（`get_`）**: `echo` が必要。取得したデータを `foreach` で回して表示する。
*   **データの取り出し方**: `get_` 系は「オブジェクト」という形で返ってくるため、`$category->name` と書いて名前を取り出す。


### 具体的なコード例

#### 1. 投稿のカテゴリ名を自分でデザインして表示したいとき
```php
$categories = get_the_category(); 
foreach ($categories as $category) {
    echo esc_html($category->name); // カテゴリ名を出す
}
```
...
// 自動でリスト（ul/li）にして表示
wp_list_categories(array(
    'title_li' => '' // 見出しを表示しない設定
));
```

#### 3. 投稿が0件のカテゴリも含めて全取得したいとき
```php
$cats = get_categories(array(
    'hide_empty' => false // 0件のカテゴリも表示する
));
```

*   **ポイント**: HTMLを自由に書きたいときは `get_` 系、手っ取り早く表示させたいときは `the_` や `wp_list_` を使うのが基本です。

### カテゴリPHPでそのカテゴリを取得
その投稿のカテゴリを取得するには、`get_the_category()` を使用します。

### 基本の使い方
取得したデータは「オブジェクトの配列」なので、`foreach`で一つずつ取り出します。

```php
<?php
$categories = get_the_category(); // カテゴリ情報を取得
foreach ($categories as $category) {
    echo esc_html($category->name); // カテゴリ名を表示
}
?>
```

### よく使うプロパティ（取り出せるデータ）
`$category->〇〇` で以下の値が取得できます。

*   `$category->name` ：カテゴリの名前
*   `$category->slug` ：カテゴリのスラッグ（URLで使う文字列）
*   `$category->term_id` ：カテゴリのID番号

### 注意点
*   `get_the_category()` は「その投稿のカテゴリ」のみ取得します（全カテゴリは取得しません）。
*   取得した名前を表示するには `echo` が必要です。
*   `$category['name']` という書き方（配列形式）ではなく、`$category->name` という書き方（オブジェクト形式）を使います。
...
## 📌 `the_post_` が付く関数は少ない（基本は `the_` だけ）

【日付】2026-03-20

【結論】
`the_` だけの関数が圧倒的に多い。`the_post_` が付くのは `the_post_thumbnail()` がほぼ唯一。
「`the_post_` が付いたらアイキャッチ関係」と覚えておけばOK。

【具体例】
```
the_title()           ← the_ だけ（タイトル）
the_permalink()       ← the_ だけ（URL）
the_content()         ← the_ だけ（本文）
the_date()            ← the_ だけ（日付）
the_author()          ← the_ だけ（著者）
the_post_thumbnail()  ← the_post_ が付く（アイキャッチ画像）
```

【補足】
- ⚠ `the_post()` はデータを「準備する」関数で、出力はしない（別物）
- 💡 つまり：`the_post_` が付いたらアイキャッチ、それ以外は `the_` だけと覚える

---

## 📌 `the_` 系関数はタグをイメージ → 出てくるものをイメージして囲むか決める

【日付】2026-03-20

【結論】
関数が「何を出力するか」をイメージすれば、タグで囲むか・そのまま書くかが判断できる。
- 文字・URL → テキストだけ出てくる → タグの中に入れる
- 画像・本文 → タグごと出てくる → そのまま書く（囲まない）

★★「`the_` はタグをイメージ → 出てくるものをイメージして囲むか決める」★★

語呂の対応：
- 「タグをイメージ」 → 関数が出力するものがタグ込みかテキストだけかを頭の中でイメージする
- 「囲むか決める」 → イメージした結果、自分でタグを書くか・そのまま書くかを判断する

【具体例】
```php
// テキストだけ出てくる → タグの中に入れる
<p><?php the_title(); ?></p>         // タイトル = 文字
<a href="<?php the_permalink(); ?>"> // URL = 文字

// タグごと出てくる → そのまま書く
<?php the_content(); ?>          // 本文 = <p>ごと出てくる
<?php the_post_thumbnail(); ?>   // 画像 = <img>ごと出てくる
```

【補足】
- ⚠ `<p><?php the_content(); ?></p>` と書くと `<p>` の中に `<p>` が入って壊れる
- 💡 つまり：関数名から「何が出てくるか」をイメージすれば自然に判断できる


### 直接WordPressで the_permalink() みたいにHTML構造も作ってくれるものの共通点はわかる
`the_permalink()` のように「HTML構造（タグ）も作ってくれるもの」の共通点は以下の通り。

- **「部品」ではなく「完成品」を出す**
    - `the_content()` や `the_post_thumbnail()` は、単なるテキストやURLではなく、HTMLタグ（`<p>` や `<img>` など）を含んだ「完成されたひとかたまり」を出力する。
    - そのため、自分でタグで囲む必要がない。

- **役割の違い**
    - **情報を埋め込む系**（`the_title()`, `the_permalink()`）：
        - データは「ただの文字」。
        - なので、自分が用意したタグの中に「入れる」必要がある。
    - **表示を作る系**（`the_content()`, `the_post_thumbnail()`）：
        - データ��「タグ付きのセット」。
        - なので、自分でタグを書くと二重構造になって表示が崩れる。

- **見分け方**
    - その関数が「中身だけ（文字列やパス）」を出すか、「形（タグ）」を出すかを考える。
    - 迷ったら、一度ブラウザで表示して「右クリック→検証」で、タグが二重になっていないか確認する。

### the_permalink()ってタグ補完するんだっけ？
- `the_permalink()` は「URL（ただの文字）」しか出さない。
- そのため、自分で `<a>` タグを用意して、その中に囲む必要がある。
...
- 「タグで囲むべきか」の判断基準：
    - `the_permalink()` や `the_title()` → 「文字（材料）」なので**タグで囲む**。
    - `the_content()` や `the_post_thumbnail()` → 「タグ付き（完成品）」なので**囲まない**。
...
## 📌 `single_cat_title()` → 今表示しているカテゴリー名を出力する

【日付】2026-03-20

【結論】
`single_cat_title()` は category.php で使う関数。
今表示しているカテゴリーの名前を出力する。`echo` 不要（`the_` 系と同じ動き）。
...
【補足】
- ⚠ `echo` は不要（内部で出力してくれる）
- 💡 つまり：category.php でカテゴリー名を表示したいときはこれ1択
- 💡 つまり：一覧から詳細ページへのリンクは必ずこのパターンを使う

### single.php はファイル1つ・表示は投稿ごとに変わる（型紙イメージ）

【日付】2026-03-20

ファイルは1つだが、WordPressが投稿ごとにデータを差し替えて表示する。
型紙が1枚あって、そこに投稿ごとのデータを流し込んでいるイメージ。

```
single.php（ファイルは1つ）
  ↓ WordPressが投稿ごとに中身を差し替えて表示

投稿①「ランニングシューズ」→ single.php に商品Aのデータを入れて表示
投稿②「レザーバッグ」      → single.php に商品Bのデータを入れて表示
投稿③「キャップ」          → single.php に商品Cのデータを入れて表示
```

`the_title()` や `the_content()` が「今この投稿のデータを取ってきて」という命令なので、同じコードでも表示される内容が変わる。
category.php も同じ考え方（カテゴリーごとにデータが変わる）。


### カテゴリPHPのタイトルを取得
カテゴリーのタイトル（名前）を表示する方法は以下の通りです。
...
### ポイント
*   **`echo` は不要**：命令を書くだけで、勝手に名前を表示してくれます。
*   **場所**：`category.php` というファイルの中で使います。
*   **動き**：今見ているカテゴリーの名前（例：靴、バッグなど）を自動で判断して表示してくれます。

### カテゴリPHPとは
### カテゴリPHP（category.php）とは
WordPressにおいて、特定のカテゴリーに属する記事一覧を表示するための「型紙（テンプレートファイル）」のこと。

*   **役割**: 「靴」や「バッグ」など、カテゴリー別の記事一覧ページを作るための専用ファイル。
*   **仕組み**: 1つのファイルで、どのカテゴリーが開かれても自動的にそのカテゴリー専用のタイトルや記事リストを表示する。
*   **名前の取得方法**: `single_cat_title()` を使う。
...
### 重要なルー���
*   **`echo` は不要**: `single_cat_title()` を書くだけで勝手に画面に表示される。
*   **型紙としての動き**: 
    *   「靴」カテゴリーを表示 → 「靴」という文字を表示
    *   「バッグ」カテゴリーを表示 → 「バッグ」という文字を表示
    *   （中身のデータをWordPressが勝手に差し替えてくれる）

### カテゴリPHPでそのカテゴリを取得
- カテゴリーページ（category.php）でカテゴリー名を表示する関数：`single_cat_title()`
- 書き方：`<?php single_cat_title(); ?>`
- 注意点：`echo` は不要。書くだけで自動的に画面に名前が表示される。
- 特徴：今開いているカテゴリーのページに合わせて、表示される名前が自動で切り替わる。
