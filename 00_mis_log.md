...
- 固定ページは contact / confirm / thanks の3つ作る → thanks を忘れやすい- the_content() → 固定ページの本文エリアを出力 → 管理画面から文言変更できる。echo でハードコードすると管理画面から変更不可- All in One SEO 設定場所 → サイトタイトルは「設定→一般」・区切り文字とメタは「AIOSEO→Search Appearance」で設定する