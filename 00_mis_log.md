...
　
  ...
  ✅0504 - `esc_url( get_category_link($cate) )` → URLを取得（hrefの中）　→<a href="' . esc_url( get_category_link($cate) ) . '">ここはサイドの"" でかこまれているる
...
（  - `the_post_thumbnail()` → `<img>` ごと出る
  ...
  ✅0504 ※なお基本以下の通りthe_XXXだが例外がある。サムネイル
the_title()           ← the_ だけ
...
✅0503 ✅position: absolute する要素は親の子にしない → 兄弟要素にするとSP切り替えで static に戻すだけで縦並びになる(子要素にするとあとでSPのとき、分けて処理できないのでやっかい。)
...
✅0503 - ✅ transition → 値が変わるときになめらかに動かす。transition: プロパティ名 時間 イージング の形
...
📝 【問題】Gridレイアウトでカードの高さが揃わない際、文字量に応じて柔軟に高さを調整するために推奨されるCSSプロパティはどれか？
   
   Q: ちょっとイメージがわかないから、SVGで説明してもらえますか
   A: 要するに、`min-height`を指定することで、コンテンツが少ない場合でも親要素の高さが維持され、多い場合は自動で拡張される仕組みです。
   
   ```svg
   <svg width="200" height="100">
     <rect x="10" y="10" width="80" height="80" fill="#ddd" />
     <text x="20" y="50" font-size="12">最小高さ</text>
     <rect x="110" y="10" width="80" height="80" fill="#bbb" />
     <text x="120" y="40" font-size="12">文字が</text>
     <text x="120" y="55" font-size="12">増えると</text>
     <text x="120" y="70" font-size="12">伸びる</text>
   </svg>
   ```
   
   | 状態 | 挙動 |
   |---|---|
   | 文字が少ない | `min-height`の値まで維持される |
   | 文字が多い | コンテンツ量に合わせて自動拡張される |
...
📝 【問題】ブラウザのタブやブックマーク一覧などで、サイトを識別するために表示される小さなアイコンを何と呼びますか？
   
   Q: これ、正解は何？
   A: 要するに、その小さなアイコンは「ファビコン（favicon）」と呼びます。
   これは「favorite icon」の略称であり、Webサイトの運営者が独自に設定する画像ファイルです。
   HTMLファイルの`<head>`タグ内に以下のように記述することでブラウザに認識させます。
   
   ```html
   <link rel="icon" href="favicon.ico">
   ```
   
   設定の有無による表示の違いは以下の通りです。
   
   | 項目 | 設定あり | 設定なし |
   |---|---|---|
   | アイコン | オリジナル画像 | ブラウザの標準アイコン |
   | 目的 | 視認性の向上 | 識別不可 |
   
   ブラウザはサイトにアクセスする際、ルートディレクトリに配置されたファイルを自動的に読み込みます。
...
✅0503 以下の２番目で/mainにフレックス１を設定してない場合は、margin-top: autoする必要がある。mainにflex1を設定すれば伸びるので
...
📝 【問題】WordPressで投稿タイプ「news」のアーカイブページURLを有効にするために、functions.phpで設定すべきプロパティはどれか？
   
   Q: これ、カスタム投稿タイプのこと言ってるよね？普通の投稿タイプの場合はどうなるの？
   A: 要するに、普通の投稿（post）は最初からアーカイブ機能が有効ですが、カスタム投稿タイプは設定で明示する必要があります。
   
   カスタム投稿タイプで `has_archive` を設定しないと、URLにアクセスしても404エラーとなります。
   
   ```php
   register_post_type('news', array(
       'has_archive' => true, // または 'news'
       'public'      => true,
   ));
   ```
   ...
   |---|---|
   | デフォルト（post） | 設定不要（有効） |
   | カスタム投稿 | `has_archive => true` が必要 |
   
   処理の流れは以下の通りです。
   ```
   register_post_type実行 ↓
   has_archive: true/文字列 指定 → URL生成 ✅
   has_archive: false/未指定 → URL生成 ❌
   ```
   
   
   ※要するに、通常の投稿タイプは、functions.phpになにもかかなくていい。(もしもメニューに表示オプションから登録すれば、functions.phpにかかない。「正規の方法」)
...
✅0503 - `section { flex: 1 }` + `pagination` が同じ flex コンテナにあると section が全スペースを取って pagination がはみ出す → section から `flex: 1` を削除する
- 開発中の「記事0件」白い空白は本番では起きない → デザインカンプは記事がある状態で設計されているため気にしなくてよい


- 検証ツールで対象要素を確認 → 正しいセレクタに `display: flex` を当てることで正確な位置に配置できる

✅0503 - ✅[0429] ブラウザのデフォルトスタイル（`ul/li` は縦・点つき）は検証ツールで取り消し線で確認できる → `display: flex` + `list-style: none` で上書きする
...
✅0503 - `archive.php` はカテゴリーURL（`/category/スラッグ/`）でアクセスしたときに呼ばれる
...
📝 【問題】WordPressにおいて、指定したカテゴリのスラッグを持つ投稿を取得するために使用する関数はどれか。
   
   Q: これはカテゴリも絞れるし、他のことも絞れるってことだよね。他の何が絞れるの？
   A: 要するに、`WP_Query`は投稿タイプや公開日、著者など、データベース上のほぼすべての属性で投稿をフィルタリングできるクラスです。
   
   指定可能な主なパラメータは以下の通りです。
   
   | パラメータ | 絞り込み内容 |
   |---|---|
   | post_type | 投稿タイプ（固定ページやカスタム投稿など） |
   | author | 投稿者ID |
   | date_query | 投稿日時や期間 |
   | meta_query | カスタムフィールドの値 |
   ...
   クイズの回答である`WP_Query`は、これらを組み合わせることで「特定の条件に一致する投稿リスト」を柔軟に生成する仕組みです。
...
✅0506 - `width: 100%` はブロック要素（div・p・h1等）には書かなくていい（デフォルトで親幅いっぱい）→ `img` `a` `span` などインライン要素には必要
...
✅0503 - VS Code で WordPress関数に青線（intelephense警告）→ エラーではない・WordPressのスタブがないだけ → ブラウザでは正常動作する
...
✅0504 1. デフォルトのアーカイブは、functions.php（set_post_archive）
でも管理画面からでも登録できる
...
✅0503 ・wp_nav_menu()	選択中のコード	管理画面でメニューを自由に編集できる
・get_post_type_archive_link()	直接URLを書く	PHPに直書き・固定


✅0503 - ❌[0430] カテゴリーアーカイブのURL取得 → get_term_link('スラッグ', 'category') [プレビュー](http://localhost:54321/preview-20260429-072010.svg) svg/get_term_link_vs_category_link.svg
...
✅0503 - CF7のフォーム表示は `the_content()` で行う → 固定ページ本文にショートコードを貼り、PHPで the_content() を呼ぶ（問い合わせページ・page-contact.phpでつかう）
...
✅0503 - display: grid + gap のみ（grid-template-columns なし）→ flex-direction: column + gap と同じ縦並べになる
...
✅0503 - `setInterval(処理, ミリ秒)` → 第一引数が実行する処理（関数）、第二引数が繰り返す間隔。1秒=1000ms。setTimeout は1回だけ、setInterval はずっと繰り返す トップ画面での画面の切り替えなどで利用する
