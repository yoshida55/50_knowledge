...
## \* [ ] Span タグと, h2p タグとかのタイトルの付け方 h2 と span タグと h2 　 p タグの使い訳
...
```html
➁menu-content (flex 親) 　➂meny_item（小　coffee等「←」flex 親） 　　➃dl(商品名)
⑤dt（価格） ➂.menu-item（小　food等「⇀」flex 親）
```
