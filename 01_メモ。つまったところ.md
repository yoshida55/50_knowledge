...
`dl` `dt` `dd` タグでマークアップ
  ...
  <dl class="news_List">
    <div class="news_part">
      <dt>2021.01.01</dt> ⇀タイトル
      <dd>タイトルタイトルタイトルタイトル</dd>
    </div>
    <!-- ...他の項目... -->
  </dl>

  （dt,ddは基本意味合いが同じ時につかう）
  ![](images/2026-01-08-07-11-56.png)
  ![](images/2026-01-06-19-41-24.png)

  ※ h2,pタグと思われるかもしれない。
h2 / p の場合: 「大きな見出し」と「ただの文章」という関係。バラバラでも成立してしまう。
...
2. おかしいと思ったらこの1行だけ打ってください。

git remote -v
・「あれ？知らないURLだ」→ フォルダ間違い確定
・「合ってる」→ 他の原因

※作業したいフォルダ（今回は 50_knowledge）を直接 VS Codeで開く。
...
## \* [ ] Span タグと, h2 p タグとかのタイトルの付け方 h2 と span タグと h2 　 p タグの使い訳
...
<h2 class="section-title">
  <span class="ja">新しい価値の創</span>
  <span class="en">Create Values</span>
</h2>

→ 両方とも「見出し」の一部。同じ意味を日英で言い換えているだけ

H2 タグを使うときは、div タグで囲む。

```html
<div class="Fashion_title_list">
...
## タイトル(title)と詳細(description)で囲む場合以下の構成する（書き直し、ただキーわーｄがおおいので上記の文章にここに記載されているキーワードを足す・タイトル(title)と詳細(description)など）

div で囲んで　 h2 と p タグにする
...
```html
➁menu-content (flex 親) 　
  ➂meny_item（小　coffee等「←」flex 親） 　　➃dl(商品名)
⑤dt（価格） 
  ➂.menu-item（小　food等「⇀」flex 親）
```
...
background-size: 150%; /_ 画像のサイズを調整 _/
...
## widthを指定して、heightをautoにすると自然なかたちになる。
# ヘッダータイトル　固定幅　画像指定

とくにボックスに文字があてはまらない時など利用するとよい。またheightは基本しようしないので、
その理由からも多用するとよい。
